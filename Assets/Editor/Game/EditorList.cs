...
public class TestListInspector : Editor
{
    ...
    private void DrawNameElement(Rect rect, int index, bool selected, bool focused)
    {
        ...
        {
            m_NameList.elementHeight = 40;
            EditorGUI.HelpBox(new Rect(rect.x + 110, rect.y, rect.width / 4, EditorGUIUtility.singleLineHeight), "从储存列表中按照ToggleGroup值获取", MessageType.None);
            EditorGUI.PropertyField(new Rect(rect.x, rect.y + 20, 100, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("Name"), GUIContent.none);
        ...
        if (ShowType == GetTypeValue.GetFromToggle)
        {
            m_NameList.elementHeight = 40;
            ...
            EditorGUI.PropertyField(new Rect(rect.x, rect.y + 20, 100, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("Name"), GUIContent.none);
            EditorGUI.PropertyField(new Rect(rect.width - 100, rect.y + 20, 100, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("toggle"), GUIContent.none);
        }
